...
export const POST = async (req: Request) => {
    ...
    try {
        if (header === "importcustlist") {
            ...
            const getId = await supplierModal.find({}, { id: 1, _id: 0 });
            ...
            await supplierModal.insertMany(objectCreate);
    ...
    if (header == "addcust") {
        try {
            if (request) {
                await supplierModal.create(request);
...
export const GET = async (req: Request) => {
    ...
    try {
        await connectDB();
        if (header === "doc-count") {
            const response = await supplierModal.find({}, { id: 1, _id: 0 });
            if (response.length) {
            ...
            const response = await supplierModal.find();
            console.log(response);
...
export const DELETE = async (req: Request) => {
    ...
    if (header == "deleterow") {
        ...
        try {
            const response = await supplierModal.deleteMany({ _id: { $in: id } })
            if (response) {
                console.log(response);
    ...
    if (header == "deletesingle") {
        ...
        try {
            ...
            if (id) {
                const response = await supplierModal.deleteOne({ _id: id });
...
export async function PUT(req: Request) {
    ...
    try {
        await connectDB();
        const response = await supplierModal.updateOne({ _id: data._id }, data)
