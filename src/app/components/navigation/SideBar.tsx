  ...
  return (
    <div className="w-[280px] min-h-screen bg-[--primary] rounded-lg">
      <div className="flex flex-col gap-2 items-center px-2">
        ...
        <Accordion
          type="single"
          className="w-full flex flex-col gap-1 items-center py-2"
          collapsible
        >
          {navItems.map((item, index) => {
            if (!item.children) {
              return (
                <Link
                  key={index}
                  className={cn(
                    "flex items-center w-full gap-2 text-white no-underline bg-white/[.2] py-2 hover:bg-white/[.3] rounded-lg px-2",
                    item.key === pathname
            ...
            return (
              ...
              >
                <AccordionTrigger
                  className={cn(
                    "text-white flex items-center justify-between w-full no-underline bg-white/[.2] py-2 hover:bg-white/[.3] rounded-lg px-2"
                  )}
                >
                    ...
                    {item.children?.map((child, index) => {
                      return (
                        <Link
                          key={index}
                          className={cn(
                            "px-2 py-2 w-full bg-white/[.2] rounded-md text-white hover:bg-white/[.3]",
                            child.key === pathname
                              ? "text-gray-800 font-bold bg-white/[.8] hover:bg-white/[.8]"
                              : ""
                          )}
                          href={child?.key ? child.key : ""}
                        >
                          {child.label}
                        </Link>
                      );
